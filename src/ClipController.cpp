    ...
    namespace app {
        ...
        void draw()
        {
            for (const auto &clipname : publicLayers) {
                if (!clipManager->exists(clipname)) continue;
                clips[clipname]->draw();
            };
...
//            };
        ...
        void mousePressed(int x, int y, int button)
        {
            ...
            loopier::ClipPtr clip = clipManager->getClipAt(x,y);
            if (clip && x == ofGetPreviousMouseX() && y == ofGetPreviousMouseY()) {
                bool selected = clip->isSelected();
                if (!ofGetKeyPressed(OF_KEY_SHIFT)) clipManager->deselectAllClips();
                if (selected)   clipManager->deselectClip(clip->getName());
                else            clipManager->selectClip(clip->getName());
            }
...
//            moveClipTo(clipname, ofGetWidth() / 2, ofGetHeight() / 2);
    ...
    namespace cv {
        ...
        void setInputClip(string clipname)
        {
            ofLogVerbose() << __PRETTY_FUNCTION__ << " needs implementation";
            if (!clipManager->exists(clipname)) return;
            if (!clipManager->exists("cv")) return;
            clips["cv"]->setInputClip(clipManager->getClip(clipname));
...
//            clips["cv"]->setParent(clips[clipname]);
        ...
        {
            ofLogVerbose() << __PRETTY_FUNCTION__ << " needs implementation";
//            clipManager->setClipColor("cv", color);
        }
